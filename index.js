...
function pontoCraps(ponto) {
  console.log(`Seu ponto é ${ponto}. Continue jogando até tirar ${ponto} novamente. Não tire 7 antes!`);
  while (true) {
    let numero = rolarDados();
    console.log(`Você tirou ${numero}`);
    if (numero === 7) {
...
function jogarNovamente() {
  let opcao = prompt('Digite "sim" se quiser jogar novamente, Digite "não" se quiser interromper:');
  if (opcao.toLowerCase() === "sim") {s
    craps();
  } else {
